...
function ProfilePage() {
  ...
  async function upload(file?: File) { if (!file) return; setStatus("Uploading photo…"); const response = await apiFetch(`/api/me/profile-photo?slot=${photoSlot}`, { method: "PUT", headers: { "Content-Type": file.type }, body: await file.arrayBuffer() }); if (!response.ok) { setStatus(await apiErrorMessage(response, "Could not upload photo.")); return; } await load(); setStatus("Photo uploaded."); }
